...
exports.init = async function () {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (_.isObject(process.env)) {
        for (var v in process.env) {
          const varName = v.toUpperCase();
          const varValue = process.env[v].toLowerCase();

          if (envVars.hasOwnProperty(varName)) {
            exports[varName] = varValue;
            console.log(
              `Используется переменная окружения ${varName}: ${varValue}`
            );
          }
        }
      }
      ...
      for (var v in envVars) {
        if (envVars[v] && !this.hasOwnProperty(v)) {
          console.log(
            `Переменная окружения ${v} не найдена, используется значение по умолчанию: ${envVars[v]}`
          );
          exports[v] = envVars[v];
        }
      }
      ...
      console.log(
        'Инициализация переменных окружения завершена с ошибкой',
        err
      );
